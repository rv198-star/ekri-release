...
> **大型 AI Coding 项目的必备工程知识辅助技能：让 AI 先了解项目，再决定复用、扩展还是新增。**
...
EKRI Skills 面向 AI Agent，对业务项目的实现保持**无侵入**：
...
`ekri-init`、`ekri-refresh` 和 `ekri-query` 可以读取项目、查询已有工程知识、判断哪些知识需要补充或重新确认，但不会借“初始化”或“刷新”的名义修改业务实现。
...
这部分内容属于**项目工程知识资产**，不是业务源码。

只有在用户明确授权后，`ekri-init` / `ekri-refresh` 才允许把确认后的 EKRI 知识持久化到这个目录；未经授权时，只提供只读分析结果和待确认的知识建议。

`.EKRI/project/**` 建议作为项目资产纳入 Git 版本管理，使工程知识能够随代码版本一起演化和复用。`ekri-query` 始终保持只读。
